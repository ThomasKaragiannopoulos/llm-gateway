...
- `app/services/contracts.py` defines the request-side contracts used by middleware and services so the service layer does not depend on FastAPI objects.
- `app/services/policy.py` owns API-key auth, rate-limit, and quota policy decisions. Middleware translates HTTP requests into policy inputs and outputs.

## Request Lifecycle

1. Middleware assigns request IDs, logs structured request metadata, and delegates auth/rate-limit/quota checks to policy services.
2. Routers stay thin and translate transport requests into typed request contexts before delegating.
3. Services own routing decisions, provider fallback, caching, persistence, and usage accounting.
4. Repositories expose intention-revealing operations for request lifecycle and key usage instead of pushing row mutations into callers.
...
- `app/services/observability.py`: structured decision logging and Prometheus query helpers.
...
- Chat and policy services only consume typed request contracts. They should not import `fastapi`, `Request`, or router helpers directly.
...
- Request persistence captures routing, cache, and failure metadata so operational debugging does not depend on logs alone.
